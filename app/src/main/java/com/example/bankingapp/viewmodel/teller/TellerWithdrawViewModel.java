...
/**
 * Teller cash withdrawal (F15, F16, F17 + F55 SAVING rules).
 */
public class TellerWithdrawViewModel extends ViewModel {
    ...
    public TellerWithdrawViewModel(UserRepository users,
    ...
    public void withdraw(int tellerUserId, String accountNumber, String amountStr, String description) {
        withdraw(tellerUserId, accountNumber, amountStr, description, System.currentTimeMillis());
    }

    public void withdraw(int tellerUserId, String accountNumber, String amountStr, String description, long nowMillis) {
        ValidationResult v;
        ...
        executors.io().execute(() -> {
            ...
            if (account == null) {
                state.postValue(UiState.error("F15", "Account not found"));
                return;
            }
            ...
            if (ba != BusinessRules.BranchAccessResult.ALLOWED) {
                state.postValue(UiState.error("F17", "Account is not in your branch"));
                return;
            }
            ...
            Transaction saved = transactions.save(new Transaction(
                    0L, null, accountNumber, amount, 0L,
                    TransactionType.WITHDRAW, description, nowMillis,
                    tellerUserId,
                    TransactionStatus.SUCCESS, account.getBalance()));

            audit.log(tellerUserId, AuditLogger.ACTION_WITHDRAW, "transactions", null,
